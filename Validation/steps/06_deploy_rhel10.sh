...
cd $CURRENT_DIR/../http
wget -nc https://repo.almalinux.org/almalinux/10/isos/x86_64/AlmaLinux-10-latest-x86_64-dvd.iso
cd $CURRENT_DIR

ssh -o StrictHostKeyChecking=no bluebanquise@$mgt1_ip wget -nc http://$host_ip:8000/AlmaLinux-10-latest-x86_64-dvd.iso

ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null bluebanquise@$mgt1_ip <<EOF
set -x
sudo mkdir -p /var/www/html/pxe/netboots/redhat/10/x86_64/iso
sudo mount /var/lib/bluebanquise/AlmaLinux-10-latest-x86_64-dvd.iso /var/www/html/pxe/netboots/redhat/10/x86_64/iso
