...
if [ ! -f $package_path ]; then
    ...
    if [ $distribution_architecture != 'arm64' ] && [ $distribution_architecture != 'aarch64' ] ; then
        ...
        if [ ! -f $working_directory/sources/memtest86plus/README.md ]; then
            mkdir -p $working_directory/sources/memtest86plus/
            cd $working_directory/sources/memtest86plus/
        ...
        if [ -z $memtest86plus_release ]
        then
            memtest86plus_release=$(git rev-list HEAD --count)
        fi
        ...
        export memtest86plus_version=$memtest86plus_version.$memtest86plus_release
        cp -a $working_directory/sources/memtest86plus ./memtest86plus-$memtest86plus_version
        # $working_directory/sources/memtest86plus-$memtest86plus_version.tar.gz .
