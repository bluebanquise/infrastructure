set -x
# if [ "$1" == 'yes' ]; then
podman run --rm -v $HOME/CI/repositories/ubuntu2204/arm64/bluebanquise/:/repo/ ubuntu:22.04 /bin/bash -c ' \
    set -x ; \
    apt-get update ; \
    ...
    '
# else
# podman run --rm -v $HOME/CI/repositories/ubuntu2004/arm64/bluebanquise/:/repo/ ubuntu:20.04 /bin/bash -c ' \
#     set -x ; \
#     apt-get update ; \
...
#     wget -np -nH --cut-dirs 5 -r --reject "index.html*" http://bluebanquise.com/repository/releases/latest/ubuntu2004/arm64/bluebanquise/ ; \
