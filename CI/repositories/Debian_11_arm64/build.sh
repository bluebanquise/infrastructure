set -x
# if [ "$1" == 'yes' ]; then
docker run --rm -v /home/bluebanquise/repositories/debian11/arm64/bluebanquise/:/repo/ debian:11 /bin/bash -c ' \
    set -x ; \
    apt-get update ; \
    ...
    '
# else
# docker run --rm -v /home/bluebanquise/repositories/debian11/aarch64/bluebanquise/:/repo/ debian:11 /bin/bash -c ' \
#     set -x ; \
#     apt update ; \
