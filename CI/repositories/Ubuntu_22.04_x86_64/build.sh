set -x
#if [ "$1" == 'yes' ]; then
podman run --rm -v $HOME/CI/repositories/ubuntu2204/x86_64/bluebanquise/:/repo/ ubuntu:22.04 /bin/bash -c ' \
    set -x ; \
    apt-get update ; \
    ...
    '
# else
# podman run --rm -v $HOME/CI/repositories/ubuntu2004/x86_64/bluebanquise/:/repo/ ubuntu:20.04 /bin/bash -c ' \
#     set -x ; \
#     apt-get update ; \
