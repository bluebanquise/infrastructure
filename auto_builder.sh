...
#              alternatives --set python /usr/bin/python3
    ...
    2) ######################################################################################
        ...
        if [ $distribution_architecture == 'x86_64' ]; then
            ...
            tar cvzf prometheus.tar.gz prometheus-$prometheus_version
            rpmbuild -ta prometheus.tar.gz --define "_software_version $prometheus_version"
            ...
            tar cvzf alertmanager.tar.gz alertmanager-$alertmanager_version
            rpmbuild -ta alertmanager.tar.gz --define "_software_version $alertmanager_version"
            ...
            tar cvzf node_exporter.tar.gz node_exporter-$node_exporter_version
            rpmbuild -ta node_exporter.tar.gz --define "_software_version $node_exporter_version"
            ...
            tar cvzf ipmi_exporter.tar.gz ipmi_exporter-$ipmi_exporter_version
            rpmbuild -ta ipmi_exporter.tar.gz --define "_software_version $ipmi_exporter_version"
            ...
            tar cvzf snmp_exporter.tar.gz snmp_exporter-$snmp_exporter_version
            rpmbuild -ta snmp_exporter.tar.gz --define "_software_version $snmp_exporter_version"
    ...
    3) ######################################################################################
        ...
        tar cvzf ansible-cmdb.tar.gz ansible-cmdb-$ansible_cmdb_version
        rpmbuild -ta ansible-cmdb.tar.gz --define "_software_version $ansible_cmdb_version"
        set +x
    ;;
...
# Packages versions
ipxe_bluebanquise_version=1.1.3
...
cp -a packages $working_directory/sources/bluebanquise
exit
